...
1º validamos campos, con validacio.js
2º insertamos video en DB tabla [datos_video], en Videos::insert_video, usando clase Videos de Videos.php
3º insertamos página en DB tabla [paginas], Pages::insert_page, clase Pages de pages.php
4º usando index_template.php, reemplazamos variables: $placeholders = array("{video_id}", "{platform_video_id}", "{page_title}", "{new_h3}", "{client_name}", "{cat_1}", "{cat_2}", "{cat_3}", "{cat_4}", "{new_page_id}");


	version 		Fecha 			Descripcion
	v1.1 			21-08-2017 		Categorias: solo 1 obligatoria, cuento numero y foreach.
*/
...
//require_once ($_SERVER['DOCUMENT_ROOT'] . '/videolayers/v0.7/mysql/models/' . 'videos.php');
		...
		// layers
		/* v1.0
		$cat_1 = $categorias[0];	//$_POST['cat_1'];
		$cat_2 = $categorias[1];	//$_POST['cat_2'];
		$cat_3 = $categorias[2];	//$_POST['cat_3'];
		$cat_4 = $categorias[3];	//$_POST['cat_4'];
		*/
		// v1.1
		$categorias = array_filter($_POST['categories']);	// array_filter filtra solo las entradas no-nulas.
		$cat_num = count($categorias);
		//for($i=0; $i < $cat_num; $i++){
		//	$cat[$i] = $categorias[$i];
		//}
		// page
...
$debug = 'video_id: ' . $video_id . ', $_POST["new_video_id"]: ' . $_POST['new_video_id'];
		...
		// Ahora debo reemplazar contenido:
			...
			*/
			// v1.1
			$data['cat_num'] = $cat_num;		// Anyadido en v1.1
			if ($cat_num == 0) {
				$data['cat_1'] = 'Comments';	// default value.
			}
			else {
				for($i=0; $i < $cat_num; $i++){
					$cat_name = $i + 1;
					$data['cat_'.$cat_name] = $categorias[$i];
				}
			}
			
			$data['new_page_id'] = $new_page_id;

			// v1.0 $placeholders = array("{video_id}", "{platform_video_id}", "{page_title}", "{new_h3}", "{client_name}", "{cat_1}", "{cat_2}", "{cat_3}", "{cat_4}", "{new_page_id}");
			// v1.1
			$placeholders = array("{video_id}", "{platform_video_id}", "{page_title}", "{new_h3}", "{client_name}");
			$placeholders[] = "{cat_num}";		// Anyadido en v1.1
			if ($cat_num == 0) {
				$placeholders[] = "{cat_1}";	// default
			}
			else {
				for($i=0; $i < $cat_num; $i++){
				$cat_name = $i + 1;
				$placeholders[] = "{cat_".$cat_name."}";
				}
			}
			$placeholders[] = "{new_page_id}";
...
echo('
						...
						<div class="container">
							<div id="form">
								...
								<p><br /><br /><br /></p>
								<p>Click <a href="' . $link_main_page . '">here</a> to add another page.</p>
								<p><br /><br /><br /></p>
								<p>Click <a href="index.php">here</a> to go back to the Main page.</p>
...
<script src="js/form-validacions.js"></script>
										...
										<div class="addpage_contenido tab-pane active" id="addpage_contenido_video">
											<div class="row">
												...
												<div class="col-xs-6 addpage_contenido_video_imagen" id="addpage_contenido_video_imagen_1">
													<img src="img/form_video_vimeo.jpg" width="488" height="300" />
												</div>
										...
										<div class="addpage_contenido tab-pane" id="addpage_contenido_categorias">
											<div class="row">
												<div class="col-xs-6">
													...
													<div id="addpage_contenido_categorias_names">													
														<div class="form-group">
															...
															<input type="text" id="cat_1" name="categories[]" class="form-control" placeholder="Name for the 1st tag, subject, topic" />
														</div>
													</div>
													<a id="btn_add_categ" href="#">Add another category <i class="fa fa-plus"></i></a>
												</div>
...
$( document ).ready( function(){
	$( "#btn_add_categ" ).click( function(e){
	    // custom handling here
	    e.preventDefault();
		var num_cats = $("#addpage_contenido_categorias_names input").length;
		var new_num_cats = num_cats + 1;
		var new_categ = $( "<div class=\'form-group\'><label for=\'cat_"+new_num_cats+"\'>Category Name</label><input type=\'text\' id=\'cat_"+new_num_cats+"\' name=\'categories[]\' class=\'form-control\' placeholder=\'Write here your category name\' /></div>");
		$("#addpage_contenido_categorias_names").append(new_categ);
	});
});
</script>
