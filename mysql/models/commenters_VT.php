...
class Commenters {
	...
	public static function getAllRoles() {
		$sql_pdo = "select * from voc_user_roles order by role_id asc";
		//$output = array();		// peta si es un array.
		
		if (!isset($db)) {
			$debug_conexio = 'no estava definida db (commenters - getAllRoles)';
			require(SCRIPT_DIR . "connect.php");					// con require_ONCE no funciona (porque db_connect lo 'machaca').
		}
		
		$stmt = $db->prepare($sql_pdo);
		try{
			$debug_conexio .= 'dins de try (commenters - getAllRoles)';
			//$stmt->execute();
			$stmt->execute();
			...
			if ( count($result) ) { 
				foreach($result as $row) {
					//print_r($row);
					$std = new stdClass();
					$std->role_id = $row['role_id'];
					$std->role_name = $row['role_name'];
					$std->description = $row['description'];

					$std->debug_count = count($result);
					$std->debug = 'commentersphp  - getAllRoles if query ok';
					$debug_pagesphp = 'estoy en commentersphp - getAllRoles if query';
			...
			echo 'catch en commentersphp - getAllRoles!!, y debug_conexio: ' . $debug_conexio;
	...
	public static function getAllUsers() {
		$sql_pdo = "select * from members order by id desc";
		//$output = array();		// peta si es un array.
		
		if (!isset($db)) {
			$debug_conexio = 'no estava definida db (commenters - getAllUsers)';
			require(SCRIPT_DIR . "connect.php");					// con require_ONCE no funciona (porque db_connect lo 'machaca').
		}
		
		$stmt = $db->prepare($sql_pdo);
		try{
			$debug_conexio .= 'dins de try (commenters - getAllUsers)';
			//$stmt->execute();
			$stmt->execute();
			$result = $stmt->fetchAll();
			if ( count($result) ) { 
				foreach($result as $row) {
					...
					$std->debug_count = count($result);
					$std->debug = 'commentersphp  - getAllUsers if query ok';
					$debug_pagesphp = 'estoy en commentersphp - getAllUsers if query';
					$debug_conexio .= 'rowCount > 0, y ' . $std->debug;
					$output[] = $std;
				}
			 }
			...
			echo 'catch en commentersphp - getAllUsers!!, y debug_conexio: ' . $debug_conexio;
			echo 'Error: ' . $e->getMessage();
			return false;
		}
	...
	public static function getAllMembers() {			// añadir parámetro de entrada en que sólo se listen los usuarios para este cliente. --> $client_id = "..."
		//$sql = "select * from members where id=$userId";
		$sql_pdo = "select * from members where role_id>=3 order by id desc";
		//$output = array();		// peta si es un array.
		
		if (!isset($db)) {
			$debug_conexio = 'no estava definida db (commenters - getAllMembers)';
			require(SCRIPT_DIR . "connect.php");					// con require_ONCE no funciona (porque db_connect lo 'machaca').
		}
		
		$stmt = $db->prepare($sql_pdo);
		try{
			$debug_conexio .= 'dins de try (commenters - getAllMembers)';
			//$stmt->execute();
			$stmt->execute();
			$result = $stmt->fetchAll();
			if ( count($result) ) { 
				foreach($result as $row) {
					...
					$std->debug_count = count($result);
					$std->debug = 'commentersphp  - getAllMembers if query ok';
					$debug_pagesphp = 'estoy en commentersphp - getAllMembers if query';
					$debug_conexio .= 'rowCount > 0, y ' . $std->debug;
					$output[] = $std;
				}
			 }
			...
			echo 'catch en commentersphp - getAllMembers!!, y debug_conexio: ' . $debug_conexio;
			echo 'Error: ' . $e->getMessage();
			return false;
		}
	...
	public static function getMember($userId) {
		//$sql = "select * from members where id=$userId";
		$sql_pdo = "select * from members as m, voc_user_roles as r where m.id=:userId and m.role_id=r.role_id";
		//$output = array();		// peta si es un array.
